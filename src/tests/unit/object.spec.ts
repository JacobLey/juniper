...
export const ObjectSchemaSpec = {
    ...
    'Omit redundant properties': {

        'Composites'() {
            const objSchema = objectSchema({
                properties: {
                    abc: numberSchema(),
                    foo: true,
                    bar: false,
            ...
            const compositeSchema = objSchema
                .allOf(objSchema.required('abc'))
                .anyOf([objSchema.required('foo')])
                .if(objSchema.required('bar'), {
                    then: objSchema.minProperties(1),
                    else: objSchema.maxProperties(2),
                })
                .oneOf([objSchema.unevaluatedProperties(true)])
                .not(objSchema.properties({ x: stringSchema() }));

            expect(compositeSchema.toJSON()).to.deep.equal({
                type: 'object',
                properties: {
                    abc: { type: 'number' },
                    foo: true,
                ...
                anyOf: [{ required: ['foo'] }],
                oneOf: [{ unevaluatedProperties: true }],
                not: {
                    properties: {
                        x: {
                            type: 'string',
            ...
            expect(compositeSchema.toJSON({ openApi30: true })).to.deep.equal({
                type: 'object',
                properties: {
                    abc: { type: 'number' },
                    foo: true,
